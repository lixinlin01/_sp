...
void statement() {
    ...
    } else if (cur_token.type == TK_WHILE) { // ====== 新增 while 邏輯 ======
        next_token(); next_token(); // skip while (       
        int loop_start = quad_count; // 1. 記錄條件評估的起始行號      
        char cond[32]; expression(cond); // 2. 解析條件式
        next_token(); next_token(); // skip ) {     
        int jmp_f_idx = quad_count;
        emit("JMP_F", cond, "-", "?"); // 3. 條件為假時跳轉 (稍後回填)     
        while (cur_token.type != TK_RBRACE) statement(); // 4. 解析迴圈主體
        next_token(); // skip }      
        char start_str[10];
        sprintf(start_str, "%d", loop_start);
        emit("JMP", "-", "-", start_str); // 5. 無條件跳轉回起始行號
        sprintf(quads[jmp_f_idx].result, "%d", quad_count); // 6. 填補 JMP_F 的正確跳出位置
...
void vm() {
    ...
    while (pc < quad_count) {
        ...
        else if (strcmp(q.op, "JMP") == 0) { // ====== 新增 JMP 指令 ======
            pc = atoi(q.result) - 1; // -1 是因為 switch 結束後有統一行為 pc++
        }
